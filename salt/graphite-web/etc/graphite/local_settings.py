...
SECRET_KEY = 'QLUGceUXnrhJdvMya6YYXPMN'
...
MEMCACHE_HOSTS   = ["{{ minions['memcached']|join('","') }}"]
{% endif -%}
...
CLUSTER_SERVERS  = ["{{ minions['graphite-web']|join('","') }}"]
{% endif -%}
...
{% if minions['graphite-web'] -%}
RENDERING_HOSTS  = ["{{ minions['graphite-web']|join('","') }}"]
{% endif -%}
